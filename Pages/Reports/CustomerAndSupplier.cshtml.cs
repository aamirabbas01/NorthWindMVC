...
{
    public class CustomerAndSupplierModel : PageModel
    {
        private readonly ApplicationDbContext _context;

        public CustomerAndSupplierModel(ApplicationDbContext context)
        {
            _context = context;
        }
        public FileContentResult OnGet()
        {
            ...
            string reportPath = Path.Combine(Directory.GetCurrentDirectory(), "Reports", "CustomerAndSupplier.rdl");
            // Load the report
            LocalReport report = new LocalReport(reportPath);

            // Optionally, add data sources here

            var salesData = _context.CustomerAndSuppliersByCities.ToList();
            ...
            // Return the PDF file
            return File(result.MainStream, "application/pdf", "CustomerAndSupplierByCities.pdf");
        }
    }
}
